...
public class MetastoreConfig extends ServiceConfig
{
  ...
  {
    return resolveConnectURI(dockerHost, resolveHostPort());
  }
  ...
  {
    String connStr = connectURI.replace("<host>", host);
    return connStr.replace("<port>", Integer.toString(port));
  }

  public MetadataStorageConnectorConfig toMetadataConfig(String dockerHost)
  {
    return MetadataStorageConnectorConfig.create(
        resolveDockerConnectURI(dockerHost),
        user,
        password,
        properties);
  }
