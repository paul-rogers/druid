...
DRUID_INSTANCE=$DRUID_SERVICE
if [ -n "$SERVICE_INSTANCE" ]; then
	DRUID_INSTANCE=${DRUID_INSTANCE}-$SERVICE_INSTANCE
fi

# Assemble Java options
JAVA_OPTS="$DRUID_SERVICE_JAVA_OPTS $DRUID_COMMON_JAVA_OPTS -XX:HeapDumpPath=$LOG_DIR/$DRUID_INSTANCE $DEBUG_OPTS"
...
LOG_FILE=$LOG_DIR/${DRUID_INSTANCE}.log
echo "" >> $LOG_FILE
