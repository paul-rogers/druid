...
public class CatalogResourceTest
{
  ...
  public void testCreate()
  {
    ...
    // Missing table name
    table = TableMetadata.newTable(TableId.DRUID_SCHEMA, null, dsSpec);
    resp = resource.createTable(table, false, postBy(DummyRequest.SUPER_USER));
    assertEquals(Response.Status.BAD_REQUEST.getStatusCode(), resp.getStatus());

    // Unknown schema
    table = TableMetadata.newTable("bogus", tableName, dsSpec);
    resp = resource.createTable(table, false, postBy(DummyRequest.SUPER_USER));
    assertEquals(Response.Status.NOT_FOUND.getStatusCode(), resp.getStatus());

    // Immutable schema
    table = TableMetadata.newTable(TableId.CATALOG_SCHEMA, tableName, dsSpec);
    resp = resource.createTable(table, false, postBy(DummyRequest.SUPER_USER));
    assertEquals(Response.Status.FORBIDDEN.getStatusCode(), resp.getStatus());

    // Wrong definition type.
    table = TableMetadata.newTable(TableId.INPUT_SCHEMA, tableName, dsSpec);
    resp = resource.createTable(table, false, postBy(DummyRequest.DENY_USER));
    assertEquals(Response.Status.BAD_REQUEST.getStatusCode(), resp.getStatus());

    // No permissions
    table = TableMetadata.newTable(TableId.DRUID_SCHEMA, tableName, dsSpec);
    resp = resource.createTable(table, false, postBy(DummyRequest.DENY_USER));
    assertEquals(Response.Status.FORBIDDEN.getStatusCode(), resp.getStatus());
    ...
    InputTableSpec inputSpec = InputTableSpec
        ...
        .column("a", "varchar")
        .build();
    table = TableMetadata.newTable(TableId.INPUT_SCHEMA, "input", inputSpec);
    resp = resource.createTable(table, true, postBy(DummyRequest.WRITER_USER));
    assertEquals(Response.Status.OK.getStatusCode(), resp.getStatus());
  }
