 ...
 * <ol>
 * <li>Skipping or partial filtering of particular dimensions or metrics entirely. Implementation could leave the body
 *  of the corresponding method empty, or implement random filtering like: <code><pre>
 ...
 *  }</pre></code></li>
 *
 *  <li>Ability to add new dimensions and metrics, possibly expensive to compute, or expensive to process (long string
 ...
 *  dimensions and metrics, could explicitly emit them in their own QueryMetrics.</li>
 *
 *  <li>Control over the time unit, in which time metrics are emitted. By default (see {@link DefaultQueryMetrics} and
 *  it's subclasses) it's milliseconds, but if queries are fast, it could be not precise enough.</li>
 *
 *  <li>Control over the dimension and metric names.</li>
 *  </ol>
 *
 ...
 *
 *
 * <h4>Types of methods in this interface</h4>
 * <ol>
 *  <li>Methods, pulling some dimensions from the query object. These methods are used to populate the metric before the
 *  query is run. These methods accept a single `QueryType query` parameter. {@link #query(Query)} calls all methods
 *  of this type, hence pulling all available information from the query object as dimensions.</li>
 *
 *  <li>Methods for setting dimensions, which become known in the process of the query execution or after the query is
 *  completed.</li>
 *
 *  <li>Methods to register metrics to be emitted later in bulk via {@link #emit(ServiceEmitter)}. These methods
 *  return this QueryMetrics object back for chaining. Names of these methods start with "report" prefix.</li>
 *  </ol>
 *
 *
 * <h4>Implementors expectations</h4>
 *
 ...
 *
 *
 * <h4>Adding new methods to QueryMetrics</h4>
 * <ol>
 * <li>When adding a new method for setting a dimension, which could be pulled from the query object, always make them
 * accept a single `QueryType query` parameter, letting the implementations to do all the work of carving the dimension
 * value out of the query object.</li>
 *
 * <li>When adding a new method for setting a dimension, which becomes known in the process of the query execution or
 ...
 * this dimension.<l/i>
 *
 * <li>When adding a new method for registering metrics, make it to accept the metric value in the smallest reasonable
 * unit (i. e. nanoseconds for time metrics, bytes for metrics of data size, etc.), allowing the implementations of
 * this method to round the value up to more coarse-grained units, if they don't need the maximum precision.</li>
 * </ol>
 *
 *
 * <h4>Making subinterfaces of QueryMetrics for emitting custom dimensions and/or metrics for specific query types</h4>
 *
 ...
 *
 *  <ol>
 *  <li>Create {@code interface SegmentMetadataQueryMetrics extends QueryMetrics} (here and below "SegmentMetadata" is the
 *  query type) with additional methods (see "Adding new methods" section above).</li>
 *
 *  <li>Create {@code class DefaultSegmentMetadataQueryMetrics implements SegmentMetadataQueryMetrics}. This class should
 *  implement extra methods from {@code SegmentMetadataQueryMetrics} interfaces with empty bodies, AND DELEGATE ALL OTHER
 *  METHODS TO A QueryMetrics OBJECT, provided as a sole parameter in {@code DefaultSegmentMetadataQueryMetrics} constructor.
 *  <p>
 *  NOTE: query(), dataSource(), queryType(), interval(), hasFilters(), duration(), queryId(), sqlQueryId(), and
 *  context() methods or any "pre-query-execution-time" methods should either have a empty body or throw exception.</li>
 *
 *  <li>Create {@code interface SegmentMetadataQueryMetricsFactory} with a single method
 *  {@code SegmentMetadataQueryMetrics makeMetrics(SegmentMetadataQuery query);}.</li>
 *
 *  <li>Create {@code class DefaultSegmentMetadataQueryMetricsFactory implements SegmentMetadataQueryMetricsFactory},
 *  which accepts {@link GenericQueryMetricsFactory} as injected constructor parameter, and implements makeMetrics() as
 *  {@code return new DefaultSegmentMetadataQueryMetrics(genericQueryMetricsFactory.makeMetrics(query));}</li>
 *
 *  <li>Inject and use SegmentMetadataQueryMetricsFactory instead of {@link GenericQueryMetricsFactory} in
 *  {@link org.apache.druid.query.metadata.SegmentMetadataQueryQueryToolChest}.</li>
 *
 *  <li>Establish injection of SegmentMetadataQueryMetricsFactory using config and provider method in
 ...
 *  can specify {@code binder.bind(ScanQueryMetricsFactory.class).to(DefaultScanQueryMetricsFactory.class)} in the extension's
 *  Guice module, if the query type is defined in an extension, e. g. ScanQuery. Or establish similar configuration,
 *  as for the core query types.</li>
 *  </ol>
