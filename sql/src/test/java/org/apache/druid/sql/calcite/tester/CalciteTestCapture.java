...
public class CalciteTestCapture
{
  ...
  private final SqlQueryPlus sqlQuery;
  ...
  )
  {
    ...
    this.expectedResults = expectedResultsVerifier.expectedResults();
    ...
    if (sqlQuery.authResult() == CalciteTests.SUPER_USER_AUTH_RESULT) {
   ...
   * Capture results.
   */
  public void results(RelDataType rowType, List<Object[]> rows)
  {
  ...
  private void writeCase(boolean includeRun, TestCaseWriter writer) throws IOException
  {
    ...
    writer.emitSql(sqlQuery.sql());
    writer.emitContext(QueryTestCases.rewriteContext(sqlQuery.context().getUserParams()));
    if (!sqlQuery.parameters().isEmpty()) {
      writer.emitParameters(sqlQuery.parameters());
    }
  ...
  {
    if (rowType != null) {
      writer.emitSchema(QueryTestCases.formatSchema(rowType));
    }
  }
  ...
  private void writeResults(TestCaseWriter writer, ObjectMapper mapper) throws IOException
  {
    ...
    writer.emitResults(QueryTestCases.resultsToJson(expectedResults, mapper));
  }
