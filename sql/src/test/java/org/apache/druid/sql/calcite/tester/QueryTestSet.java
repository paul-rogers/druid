...
public class QueryTestSet
{
  ...
  public boolean run(PlannerTestFixtureBuilder fixtureBuilder)
  {
    if (isDebugMode()) {
      setSqlCompatibleNulls(true);
      boolean sqlCompatOK = doRun(fixtureBuilder);
      setSqlCompatibleNulls(false);
      boolean classicOK = doRun(fixtureBuilder);
      return sqlCompatOK && classicOK;
    } else {
      return doRun(fixtureBuilder);
    }
  }
  ...
  public boolean doRun(final PlannerTestFixtureBuilder fixtureBuilder)
  {
    final PlannerTestFixture fixture = fixtureBuilder.buildFixture();
    boolean ok = true;
    for (TestResults testCase : results) {
      ActualResults caseResults = new QueryTestCaseConfigurator(fixture, testCase.testCase).run();
