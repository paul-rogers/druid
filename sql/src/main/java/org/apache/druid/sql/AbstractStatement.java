...
public abstract class AbstractStatement implements Closeable
{
  ...
  protected final SqlRequest sqlRequest;
  ...
  {
    return sqlRequest.context().getAsString(PlannerContext.CTX_SQL_QUERY_ID);
  }
  ...
  protected void validate(DruidPlanner planner)
  {
    ...
    plannerContext.setAuthenticationResult(sqlRequest.authResult());
    plannerContext.setParameters(sqlRequest.parameters());
    try {
      planner.validate();
    }
